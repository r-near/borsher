...
export function borshSerialize<T extends BorshSchema<unknown, string>>(
  schema: T,
  value: TypeOf<T>,
): Buffer {
...
export function borshDeserialize<T extends BorshSchema<unknown, string>>(
  schema: T,
  buffer: Uint8Array,
): TypeOf<T> {
...
export namespace b {
  export type infer<T extends BorshSchema<unknown, string>> = T extends BorshSchema<infer U, string>
    ? U
    : never
